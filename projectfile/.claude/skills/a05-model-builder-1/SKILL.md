---
name: a05-model-builder-1
description: Detailed procedure to implement, verify and document model slot 1 (per Agent 3 spec) as a fast, deterministic, mass-conserving Python module compatible with the shared optimizer and evaluator.
---
# Skill: Build model slot 1 (Agent a05)

## Purpose
Turn the specification of **slot 1** (`config/models.yaml` → `models[slot=1]`, and the Model 1 section of
`reports/03_models.qmd`) into correct, fast, tested code. You do NOT calibrate — that's `a11-optimizer-1`.
You may use default/literature parameters only for smoke tests.

## Inputs
`config/models.yaml` slot 1, `reports/03_models.qmd` (Model 1), `a03.json` downstream_notes for a05,
...
## Step 2 — Implement `src/models/model_1.py`
- Class `Model1(BaseModel)` with `model_id` = slot id, `kind` = "conceptual" or "trainable".
...
## Step 3 — Verification tests `tests/test_model_1.py` (all must pass)
...
(NOT evaluation). Plot one wet and one dry water year obs vs sim → `results/build/model_1_smoke.png`. These numbers
...
m = Model1(); q = m.simulate(forcing_df, m.defaults())
```
Update `config/models.yaml` slot 1 `status: built` (only that field of your slot).

## Handoff `a05.json` key_numbers
`n_params`, `runtime_ms_full_record`, `mass_balance_max_abs_err`, `smoke_KGE_cal_default`, `smoke_NSE_cal_default`,
`tests_passed`. downstream_notes for `a11-optimizer-1`: sensitive parameters, suspected correlations,
...
- [ ] `context/handoffs/a05.json` written (schema `context/handoffs/_TEMPLATE.json`), incl. `downstream_notes`
- [ ] Section appended to `context/CONTEXT.md`; decisions appended to `context/DECISIONS.md`
- [ ] `python scripts/state.py set a05-model-builder-1 done`
- [ ] `scripts/backup.sh a05-model-builder-1 "<message>"` → tag stored in handoff `backup_tag`

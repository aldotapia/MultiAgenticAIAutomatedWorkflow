---
name: a08-model-builder-4
description: Detailed procedure to implement, verify and document model slot 4 (per Agent 3 spec) as a fast, deterministic, mass-conserving Python module compatible with the shared optimizer and evaluator.
---
# Skill: Build model slot 4 (Agent a08)

## Purpose
Turn the specification of **slot 4** (`config/models.yaml` → `models[slot=4]`, and the Model 4 section of
`reports/03_models.qmd`) into correct, fast, tested code. You do NOT calibrate — that's `a14-optimizer-4`.
You may use default/literature parameters only for smoke tests.

## Inputs
`config/models.yaml` slot 4, `reports/03_models.qmd` (Model 4), `a03.json` downstream_notes for a08,
...
## Step 2 — Implement `src/models/model_4.py`
- Class `Model4(BaseModel)` with `model_id` = slot id, `kind` = "conceptual" or "trainable".
...
## Step 3 — Verification tests `tests/test_model_4.py` (all must pass)
...
(NOT evaluation). Plot one wet and one dry water year obs vs sim → `results/build/model_4_smoke.png`. These numbers
...
```python
from src.models.model_4 import Model4
m = Model4(); q = m.simulate(forcing_df, m.defaults())
```
Update `config/models.yaml` slot 4 `status: built` (only that field of your slot).

## Handoff `a08.json` key_numbers
`n_params`, `runtime_ms_full_record`, `mass_balance_max_abs_err`, `smoke_KGE_cal_default`, `smoke_NSE_cal_default`,
`tests_passed`. downstream_notes for `a14-optimizer-4`: sensitive parameters, suspected correlations,
...
- [ ] `context/handoffs/a08.json` written (schema `context/handoffs/_TEMPLATE.json`), incl. `downstream_notes`
- [ ] Section appended to `context/CONTEXT.md`; decisions appended to `context/DECISIONS.md`
- [ ] `python scripts/state.py set a08-model-builder-4 done`
- [ ] `scripts/backup.sh a08-model-builder-4 "<message>"` → tag stored in handoff `backup_tag`

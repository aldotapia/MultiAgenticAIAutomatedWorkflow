...
description: Detailed procedure for robust, leakage-free parameter optimization of model slot 4: multi-seed global search, convergence and identifiability diagnostics, internal validation, and final parameter selection for evaluation.
---
# Skill: Calibrate model slot 4 (Agent a14)

## Purpose
Produce the single best, well-justified parameter set (or trained weights) for model slot 4, ready for a16,
using ONLY the calibration period. Robustness > squeezing 0.001 of objective.

## Inputs
`src/models/model_4.py` + `a08.json` (builder notes), `config/optimization.yaml` per_model entry for this
...
`python -m pytest -q tests/test_model_4.py tests/test_runner.py` must pass; otherwise write to
...
- Output dir `results/calibration/model_4/`. Record library versions & hardware.
...
- Conceptual: `python -m src.optimization.runner --slot 4 --seed <s>`; save traces.
...
- Save the final-parameter simulation over the whole calibration run window to
  `results/calibration/model_4/cal_simulation.parquet` (row, date, Q_obs, Q_sim) — a16 fits ensemble weights on it.
...
## Update
`config/models.yaml` slot 4 `status: calibrated`.

## Handoff `a14.json` key_numbers
...
- [ ] `context/handoffs/a14.json` written (schema `context/handoffs/_TEMPLATE.json`), incl. `downstream_notes`
- [ ] Section appended to `context/CONTEXT.md`; decisions appended to `context/DECISIONS.md`
- [ ] `python scripts/state.py set a14-optimizer-4 done`
- [ ] `scripts/backup.sh a14-optimizer-4 "<message>"` → tag stored in handoff `backup_tag`
